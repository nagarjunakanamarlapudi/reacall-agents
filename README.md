...
Live validation on September 12, 2026 did not establish a successful investigation. OpenAI `gpt-5-mini` produced the observed plan and first specialist, then stopped with `invalid_response` before any MCP read or human review; zero receipts were written. Earlier 30-second requests timed out. Subsequent timeout and response-schema corrections have been tested offline; a completed, independently verified live run remains outstanding. The historical offline scorecard is separate from these failed live attempts.
