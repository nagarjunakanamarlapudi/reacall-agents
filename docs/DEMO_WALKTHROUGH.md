...
After these attempts, the response schemas were corrected to advertise every field the strict validator requires, including explicit empty evidence gaps and `executed=false`. The prompts now name each role's required evidence reads and require the supervisor to continue sequentially through all four responses. Offline tests inspect the actual outgoing schemas and compiled agent instructions; no verifier or receipt guard was relaxed.

One newly authorized post-correction flagship attempt used `gpt-5-mini`, 120 seconds per call, zero retries and a 600-second overall limit in a fresh isolated runtime. It failed after 314.421 seconds: the four-role plan was observed, recall intelligence completed, and sealed `get_recall`, `find_candidate_products` and `match_lots` reads succeeded. The matching task then failed; traceability and containment did not complete. The safe outcome was `semantic_failure` / `invalid_response`, escalated with no review interrupt, no fallback and zero write receipts. Nine completed model calls reported 118,229 input and 22,127 output tokens (140,356 total); cost is unavailable. This is failed-attempt evidence, not live E2E success. No further provider attempt or live evaluation was run after this failure.
