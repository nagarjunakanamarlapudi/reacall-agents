...
Live metrics remain unavailable for a successfully completed four-specialist investigation and durable HITL E2E. Failed-attempt measurements do exist. The latest authorized September 12, 2026 attempt used real OpenAI `gpt-5-mini` / `medium`: **195.583 seconds**, **eight completed model calls**, **83,995 tokens**, exact four-role plan observed, recall intelligence completed, product-lot matching failed after **one bounded correction**. The read trail contains `get_recall`, `find_candidate_products`, `match_lots`. Zero claims and zero read receipts were released; no HITL, no writes, and `passed=false`. Completed read events are not released verifier receipts. Cost is unavailable. This is failure evidence, not successful live performance. Later prompt hardening and offline tests are not another provider result.

Earlier historical attempts timed out or stopped before reads; a later pre-completion-gate attempt took 314.421 seconds, nine model calls and 140,356 tokens, also failing at matching. These are separate attempts, not combined totals. Schema/read-contract fixes, bounded child completion and subsequent prompt hardening preserve the independent verifier and receipt guards. Offline compiled-graph tests inspect the real outgoing schemas and exact middleware executables; passing those tests does not establish live E2E success.

**Choose the truthful presenter branch.** If a newly authorized live run fails, stop the live story at its sanitized failure, show which roles/reads actually completed, and state zero released claims/review/writes. Do not proceed as though the live case reached consent. For the demonstrated dual-consent sequence below, open a separate fresh runtime with `RECALLOPS_MODEL_MODE=deterministic make ui` and announce **deterministic control demonstration**. Never mix its receipts with live evidence. Only a future successful four-role/source-verified live case may continue directly into those human gates; that expected route is not currently demonstrated.
...
The table is an **expected verified-live route**, not a claim that the latest live attempt reached it. For today's evidence-backed take, show the failed live trace during Investigation, say “matching stopped after one correction; no claims or review,” then pause the clock and visibly switch to the separately labelled deterministic case before Reconciliation. The consent/receipt/closure segment demonstrates deterministic controls. Provider latency and any mode switch are outside the 4:55 narrated sequence; disclose all prerecorded traces.
...
| Is Deep Agents actually present? | Yes: OpenAI mode invokes the real `write_todos` supervisor and configures four sequential LLM specialists. The latest provider attempt observed the plan, completed recall intelligence and failed at matching after one correction; not all four completed. The committed comparative evaluation is separately `not_run_missing_credentials`; agents never receive Operations tools. |
...
The smoke calls retrieval → live service → independent source verifier directly: **it does not execute HITL or Operations**. Whole-agent live E2E additionally requires the same successful claims to enter a durable case, both human gates, version-bound simulated receipts and the closure block. A passing smoke would prove only the investigation boundary. The latest smoke failed and proves neither successful investigation nor the full human lifecycle.
