...
Latest real-provider result: `gpt-5-mini` / `medium`, exact four-lot scope, 371.975 seconds, 7 completed model calls + 1 failed event, 82,420 tokens (63,115 input / 19,305 output). The exact four-role plan and recall completion were observed. After `get_recall`, `find_candidate_products`, `match_lots`, matching's sole correction timed out at 120.006 seconds. `execution_failure` / `timeout` produced explicit `deterministic_fallback` at `review_required` / `action_review`; its verification is not live proof. The harness rejected it without approval/confirmation; Operations cases/receipts/holds/grants remained zero. Successful four-specialist/full-E2E metrics remain unavailable. Child typed-response/read validation and exact compiled hook identities do not replace the independent source verifier. The UI defaults to the four-lot scope, searchable among all 144 lots, allowing 1–64 unique choices before start and locking afterward.
