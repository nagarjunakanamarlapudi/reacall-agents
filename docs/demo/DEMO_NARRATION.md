...
This is the voice script for `RecallOps-LLM-Only-Demo.mp4`. The recording uses ElevenLabs' `Neel - Professional Indian` synthesized voice at a clear presentation pace. It shows the real OpenAI `gpt-5-mini` provider path and never substitutes a control fixture for that run. The polished cut also incorporates cropped business-domain and LLM-architecture footage from the supplied product walkthrough; its original audio and dead time are removed.

## Storyboard

| Scene | Screen | Demonstrates |
|---|---|---|
| 1 | Command Center + business-domain visual | Product, honest data boundary, and real OpenAI mode |
| 2 | High-level architecture, interleaved with walkthrough footage | LLM planning, Deep Agents, specialists, verifier, HITL, MCP, LangGraph |
